...
def test_and_explain(args, best_epoch=0):
    ...
    if args.class_number == 2:
        auc_train = calculate_auc(train_label, sigmoid(train_predict[:,1]), plot=True, path='saving/' + args.task + '/train_auc.independent.png')
        auc_valid = calculate_auc(valid_label, sigmoid(valid_predict[:,1]), plot=True, path='saving/' + args.task + '/validation_auc.independent.png')
        auc_test = calculate_auc(test_label, sigmoid(test_predict[:,1]), plot=True, path='saving/' + args.task + '/test_auc.independent.png')
