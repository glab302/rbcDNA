...
echo -en "\tTotalReadPairs\tClpperedReadPairs\tUniqNodup_Total\tUniqNodup_Properly\tIntergenic\tIntron\tExon\tCentromere\tIntergenic%\tIntron%\tExon%\tCentromere%\tCoverage\n" > Summary_Human_UniqNodupBam.log
for s in $sample
do
 c1=$(cat ./cutadapter_log/$s.log|grep "Total read pairs processed"|head -1|awk -F " " '{print $5}');
 c2=$(cat ./cutadapter_log/$s.log|grep "Pairs written (passing filters)"|head -1|awk -F " " '{print $5}');
 a1=$(samtools flagstat $s.uniq.nodup.bam -@ 70|head -1|awk -F " " '{print $1}');
 a=$[a1/2]
 b=$(samtools flagstat $s.uniq.nodup.bam -@ 70|grep properly|head -1|awk -F " " '{print $1}');
 b=$[b/2]
 inter=$(bedtools intersect -abam $s.uniq.nodup.bam -b $Genomedir/$Genome.intergenic.bed -f 1 -wa -bed|wc -l);
 inter_p=$(awk 'BEGIN{printf "%.6f\n",'$inter'/'$a1'}');
 intron=$(bedtools intersect -abam $s.uniq.nodup.bam -b $Genomedir/$Genome.intron.bed -f 1 -wa -bed|wc -l);
 intron_p=$(awk 'BEGIN{printf "%.6f\n",'$intron'/'$a1'}');
 exon=$(bedtools intersect -abam $s.uniq.nodup.bam -b $Genomedir/$Genome.exon.bed -f 1 -wa -bed|wc -l);
 exon_p=$(awk 'BEGIN{printf "%.6f\n",'$exon'/'$a1'}');
 centro=$(bedtools intersect -abam $s.uniq.nodup.bam -b $Genomedir/Centromere.bed -f 1 -wa -bed|wc -l);
 centro_p=$(awk 'BEGIN{printf "%.6f\n",'$centro'/'$a1'}');
 Coverage=$(samtools depth $s.uniq.nodup.bam | awk '{sum+=$3} END { print sum/NR}')
 perl Reads.genome.coverage.pl $Genomedir/$Genome.genome $s.uniq.bg
 A=$(tail -n1 $s.$p.uniq.bg.cov.out|awk -F "\t" '$1=="all"{printf "%.2f%%",$2*100}')

 echo -en "$s\t$c1\t$c2\t$a\t$b\t$inter\t$intron\t$exon\t$centro\t$inter_p\t$intron_p\t$exon_p\t$centro_p\t$Coverage\n" >> Summary_Human_UniqNodupBam.log
done
